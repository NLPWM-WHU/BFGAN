...
import numpy as np
import hotel.rd.model_rd as model
import argparse
...
def train():
	...
	parser.add_argument('--rating_path', type=str, default=basedir+'rd_embedding.txt',
					   help='rating_path Directory')

	parser.add_argument('--bf_path', type=str, default=basedir+'bf_embedding_new.txt',
	...
	for j in range(args.epochs):
		...
		for i in range(num_batch):
			...
			# DISCR UPDATE
			_, d_loss, gen,su = sess.run([d_optim, loss['d_loss'], outputs['generator'],merged_summary],
				feed_dict = {
			...
			# GEN UPDATE
			_, g_loss, gen,su= sess.run([g_optim, loss['g_loss'], outputs['generator'],merged_summary],
				feed_dict = {
					input_tensors['t_real_bf']: real_bfs,
					input_tensors['t_wrong_rating']: wrong_ratings,
					input_tensors['t_real_rating']: real_ratings,
		...
		if j%2==0 and j!=0:
			...
			file_bfAddtConv = open(embedpath + 'fakeBF_Epoch' + str(args.epochs) + 'Batch' + str(
				args.batch_size) + 'bfAddtConv_trainEmb(tanh2)byRatingEpoch800_vs' + str(vs) + '.txt', 'w',encoding='utf-8')
