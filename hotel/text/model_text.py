...
class GAN:
    ...
    def build_model(self):
        ...
        t_wrong_text = tf.placeholder('float32', [self.options['batch_size'], self.options['emb_dim']], name='wrong_text')
        t_real_text = tf.placeholder('float32', [self.options['batch_size'], self.options['emb_dim']], name = 'real_text_input')
      #  t_real_label= tf.placeholder('float32', [self.options['batch_size'],2], name = 'real_label_input')

      #  t_wrong_label = tf.placeholder('float32', [self.options['batch_size'], 2], name='wrong_label')  #随机噪声
        t_z = tf.placeholder('float32', [self.options['batch_size'], self.options['z_dim']])
       # t_real_label2= tf.placeholder('float32', [64,1], name = 'real_label_input')


        with tf.variable_scope(tf.get_variable_scope()):

            #随机采样文本后投入生成器 生成了行为特征fake_bf和对应匹配的文本t_text_conv UT
            fake_bf,t_text_conv,z_3 = self.generator(1,t_z, t_real_text)
            t_text_conv_sigmoid=tf.nn.sigmoid(t_text_conv)

            disc_real_bf, disc_real_bf_logits   = self.discriminator(6,t_real_bf, t_real_text)
            disc_wrong_bf, disc_wrong_bf_logits   = self.discriminator(6,t_real_bf, t_wrong_text,reuse=True)
